...
#include "DualKawaseBlurDownSamplePass.h"

#include "../../shader/vulkan/DualKawaseBlurDownSamplePass/DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_FRAG.h"
#include "../../shader/vulkan/DualKawaseBlurDownSamplePass/DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_VERT.h"

#if OPENGL_DESKTOP
#include "../../shader/gl/DualKawaseBlurDownSamplePass/DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_VERT.h"
#include "../../shader/gl/DualKawaseBlurDownSamplePass/DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_FRAG.h"
#else
#include "../../shader/gles/DualKawaseBlurDownSamplePass/DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_VERT.h"
#include "../../shader/gles/DualKawaseBlurDownSamplePass/DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_FRAG.h"
#endif
...
DualKawaseBlurDownSamplePass::DualKawaseBlurDownSamplePass(HyperGpu::GpuDevice* gpuDevice) : BasePass(gpuDevice) {
	HyperGpu::RenderEnvInfo envInfo;
	envInfo.shaderInfo = HyperGpu::ShaderInfo{
		.pSpvVertexCode	   = DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_VERT_SPV.data(),
		.spvVertexCodeSize = DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_VERT_SPV.size(),
		.pSpvFragCode	   = DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_FRAG_SPV.data(),
		.spvFragCodeSize = DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_FRAG_SPV.size(),

		.pGlVertexCode = DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_VERT_STR,
		.pGlFragCode = DUAL_KAWASE_BLUR_DOWN_SAMPLE_PASS_FRAG_STR,
	};
...
DualKawaseBlurDownSamplePass::~DualKawaseBlurDownSamplePass() {
	m_pLocalInfoBuffer0->SubRef();
	m_pLocalInfoBuffer1->SubRef();
	m_pLocalInfoBuffer2->SubRef();
}

void DualKawaseBlurDownSamplePass::SetTargetTexture(HyperGpu::Image2D *targetTexture) {
	UpdateImageBinding("texture0", targetTexture);
}

void DualKawaseBlurDownSamplePass::Draw(HyperGpu::GpuCmd *pCmd, int order) {
...
void DualKawaseBlurDownSamplePass::SetScreenSize(const Size &size) {
	m_localInfo.iResolution = { size.width, size.height };
	m_isLocalInfoDirty = true;
}

void DualKawaseBlurDownSamplePass::SetOffset(const Offset2D &offset) {
